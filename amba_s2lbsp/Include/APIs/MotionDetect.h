...
typedef struct MOTION_DETECT_PARAM
{
	...
	/// 态检测，置0表示不需要在该区域块动态检测。每一行用一个uint表示，最左边的
...
typedef struct MOTION_DETECT_RESULT
{
	...
	/// 动态检测成功，置0表示该区域块动态检测失败。每一行用一个uint表示，最左边
...
{
	uint Enabled;			///< 置1表示支持动态检测，置0表示不支持动态检测。
	uint GridLines;		///< 动态检测的区域需要划分成的列数。
	uint GridRows;			///< 动态检测的区域需要划分成的行数。
	uchar  Result;			///< 是否能得到每块区域的检测结果。
	uchar  Hint;				///< 是否能在检测成功时给予提示。
}MOTION_DETECT_CAPS;
