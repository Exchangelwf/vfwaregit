...
enum capture_size_t {
	...
	CAPTURE_SIZE_NR = 26 ///< 枚举的图形大小种类的数目。
};
...
typedef struct CAPTURE_FORMAT
{
    ...
    uchar 	    FramesPerMinute;
    uchar         Reserved;    //作字节对齐用，//add by zhangxuewu 20090812 according to sofia , for gop adjust} CAPTURE_FORMAT;
} CAPTURE_FORMAT;
/// 捕获编码能力结构
...
{
	uint ExtraStream;						///< 用channel_t的位来表示支持的功能码流。
	uint CaptureSizeMask[CAPTURE_SIZE_NR];	///< 每一个值表示相应主码流分辨率下，辅助码流对应分辨率支持的。
...
typedef enum 
{
    ...
    OSD_OVERLAY_MJ_TEMP, //球机温度
    OSD_OVERLAY_MJ_POS,  //球机坐标
    OSD_OVERLAY_MULTINFO,//多行叠加，用于多个OSD叠加到vi
    OSD_OVERLAY_NR
}OSD_OVERLAY_TYPE;
